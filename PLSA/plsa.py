    ...
    for k in range(0, K):
        ...
        for j in range(M):
            ...
            for i in range(N):
                ...
                denominator += beta[k, j]
            if denominator != 0:
                for j in range(0, M):
                    beta[k, j] = beta[k, j] / denominator
            else:
                for j in range(0, M):
                    beta[k, j] = 1.0 / M
...
def LogLikelihood():
    ...
    for i in range(0, N):
        for j in range(0, M):
            ...
            l = 0
            for k in range(K):
                l += theta[i, k] * beta[k, j]

            if l > 0:
                loglikelihood += log(l) * X[i, j]
