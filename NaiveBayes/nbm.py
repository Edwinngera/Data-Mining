...
class NB_model():
    ...
    def fit(self, train_data, train_label, vocab, if_use_smooth=True):
        ...
        self.Pr_dict = np.zeros((self.num_classes + 1, max(train_data["wordIdx"]) + 1 ))
        
        for i in range(1, self.num_classes + 1):
            class_i_df = train_data[train_data.classIdx == i]
            total = sum(class_i_df["count"])
            denomenator = total + self.num_vocab
        
            for j in range(len(class_i_df)):
                wordID = class_i_df["wordIdx"].iloc[j]
                wordcount = class_i_df["count"].iloc[j]
    
                self.Pr_dict[i][wordID] = (wordcount + 1) / denomenator
        ...
        for docIdx in range(1, len(new_dict)+1):
            ...
            for classIdx in range(1, self.num_classes+1):
                ...
                for wordID in new_dict[docIdx]:
                    try:
                        prob = self.Pr_dict[classIdx][wordIdx]
                    except:
                        prob = 0
                    if prob != 0:
                        score_dict[classIdx] += new_dict[docIdx][wordID] * np.log(prob)
