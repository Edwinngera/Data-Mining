...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        b1_mat = np.matmul(np.array([np.ones(num_inputs)]).transpose(), np.array([b1]))
        b2_mat = np.matmul(np.array([np.ones(num_inputs)]).transpose(), np.array([b2]))

        h1 = np.matmul(X, W1.transpose()) + b1_mat
        a2 = (h1 > 0).astype(int) * h1  # activation with input of h1 (ReLU)

        h2 = np.matmul(a2, W2.transpose()) + b2_mat
        scores = h2
        ...
        def MSE_loss(y_pred, y_target):
            ...
            # Transfer y_target into a matrix with same size as y_pred
            y_target_mat = np.zeros(list(y_pred.shape))

            for i in range(len(y_target)):
                y_target_mat[i][y_target[i]] = 1

            l_data = (np.linalg.norm(y_pred - y_target_mat) ** 2) / (2 * num_inputs)

            dy_pred = (y_pred - y_target_mat) / N

            return l_data, dy_pred
