...
def compute_info_gain(data, att_name, y_name):
    ...
    #Calculate the conditional entropy based on the selected attribute    

    for i in range(len(vals)):
        prob = counts[i] / total_counts
        if prob != 0:
            conditional_entropy += prob * compute_entropy(data["type"][data[att_name] == vals[i]])
...
def comput_gain_ratio(data, att_name, y_name):
    ...
    # Calculate entropy caused by split info
    att_info = 0.0
