...
# =============================================================================
#     print("Total number of data points = train_x.shape[0]: ", n)
#     print("Total number of attributes = train_x.shape[1]: ", p)
#     print("train_y.shape: ", train_y.shape)
#     
#     print("Type of train_x, train_y: ", type(train_x), type(train_y))
#     print(np.matmul(train_x.transpose, train_x))
#     print(train_x)
#     print("--------------------------------------------")
...
# =============================================================================
    ...
    # beta = np.zeros(p)
     ...
     ########## Please Fill Missing Lines Here - DONE##########
    beta = np.matmul( np.linalg.inv(np.matmul(train_x.transpose(), train_x)), np.matmul(train_x.transpose(), train_y))
...
def getBetaBatchGradient(train_x, train_y, lr, num_iter):
    ...
    for iter in range(0, num_iter):
       ...
       for i in range(n):
           ...
           # Use the gradient descent formula
           # For calculating derivatives
           xi = train_x[i]
           yi = train_y[i]
           deriv += xi * (np.subtract(np.matmul(xi.transpose(), beta), yi))
...
def getBetaStochasticGradient(train_x, train_y, lr):
    ...
    for iter in range(epoch):
        ...
        for i in range(n):
            ...
            yi = train_y[idx]
            ########## Please Fill Missing Lines Here ##########
            beta += lr * np.subtract(yi, np.matmul(xi.transpose(), beta)) * xi
    return beta
    
# predicted_y and test_y are the predicted and actual y values respectively as numpy arrays
# function prints the mean squared error value forl the test dataset
