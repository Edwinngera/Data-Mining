...
def compute_avglogL(X,y,beta):
    ...
    for i in range(n):
        xi_transpose = X[i].transpose()
        avglogL += np.subtract(y[i] * np.matmul(xi_transpose, beta),
                               np.log(eps + 1 + np.exp(np.matmul(xi_transpose, beta))))
...
def getBeta_BatchGradient(train_x, train_y, lr, num_iter, verbose):
    ...
    for iter in range(0, num_iter):
        ########## Please Fill Missing Lines Here ##########
        # beta_new = beta_old - lr * (d_L / d_beta_old)
        for i in range(n):
            yi = train_y[i]
            xi = train_x[i]
            
            num = np.exp(np.matmul(beta.transpose(), xi))
            p_xi_beta = num / (1 + num)
            for j in range(p):
                xij = xi[j]
                beta[j] += lr * (xij * (yi - p_xi_beta ))
...
def getBeta_Newton(train_x, train_y, num_iter, verbose):
    ...
    matrix = np.zeros((p, p)) # Initialize Newton-Raphson Matrix
    first_derivative = np.zeros(p)
    ...
    for iter in range(0, num_iter):
        ...
        for i in range(n):
            ...
            for j in range(p):
                ...
                for k in range(p):
                     xik = xi[k]
                     matrix[j][k] -= xij * xik  * p_xi_beta * (1 - p_xi_beta)
        ...
        for i in range(n):
            ...
            for j in range(p):
                xij = xi[j]
                first_derivative[j] += xij * (yi - p_xi_beta )
                
            # beta += 0.005 * first_derivative
            beta -= np.matmul(np.linalg.inv(matrix), first_derivative)
